...
function promedioResultadosTest(resultadosTest) {
  ...
  for(i = 0; i < resultadosTest.length; i++) {
    addi = resultadosTest [i] + addi;
  }
  return(addi / resultadosTest.length);
}
...
function mayorACien(array) {
  ...
  // Tu código:
  var arrayConMayoresA100 = [];
  for (var i = 0; i <= 200; i++) {
    if (array [i] > 100) {
      arrayConMayoresA100.push(array[i]);
    }
  }
  return arrayConMayoresA100;
}
...
function continueStatement(numero) {
  ...
  // Tu código:
  var nuevoArrayPorDiez = [];
  for (var i = 0; i < 10; i++) {
    if (i === 5) {
      continue;
    } else {
      numero = numero + 2;
      nuevoArrayPorDiez.push (numero);
    }
  }
  return nuevoArrayPorDiez;
}
